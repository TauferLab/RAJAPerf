...
__launch_bounds__(block_size) __global__
    void triad_shared_hip(Real_ptr a, Real_ptr b, Real_ptr c, Real_type alpha,
                          Index_type iend) {
...
template <size_t block_size>
void TRIAD_SHARED::runHipVariantImpl(VariantID vid) {
  ...
  if (vid == Base_HIP) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RPlaunchHipKernel((triad_shared_hip<block_size>), grid_size, block_size,
    ...
    getCout() << "\n  TRIAD_SHARED : Unknown Hip variant id = " << vid
              << std::endl;
  }
}

RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(TRIAD_SHARED, Hip, Base_HIP,
                                                  RAJA_HIP)
