...
void POLYBENCH_ATAX::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_ATAX_1));
      RPlaunchCudaKernel( (poly_atax_1<block_size>),
                          grid_size, block_size,
                          shmem, res.get_stream(),
                          A, x, y, tmp,
                          N );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_ATAX_1));

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_ATAX_2));
      RPlaunchCudaKernel( (poly_atax_2<block_size>),
                          grid_size, block_size,
                          shmem, res.get_stream(),
                          A, tmp, y,
                          N );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_ATAX_2));
...
RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(POLYBENCH_ATAX, Cuda, Base_CUDA, Lambda_CUDA, RAJA_CUDA)
