...
void POLYBENCH_FDTD_2D::runHipVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_FDTD_2D_1));
      RPlaunchHipKernel( (poly_fdtd2d_1<block_size>),
                         grid_size1, block_size,
                         shmem, res.get_stream(),
                         ey, fict, ny, t );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_FDTD_2D_1));

      FDTD_2D_THREADS_PER_BLOCK_HIP;
      FDTD_2D_NBLOCKS_HIP;

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_FDTD_2D_2));
      RPlaunchHipKernel(
        (poly_fdtd2d_2<FDTD_2D_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP>),
        nblocks234, nthreads_per_block234,
        shmem, res.get_stream(),
        ey, hz, nx, ny );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_FDTD_2D_2));

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_FDTD_2D_3));
      RPlaunchHipKernel(
        (poly_fdtd2d_3<FDTD_2D_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP>),
        nblocks234, nthreads_per_block234,
        shmem, res.get_stream(),
        ex, hz, nx, ny );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_FDTD_2D_3));

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_FDTD_2D_4));
      RPlaunchHipKernel(
        (poly_fdtd2d_4<FDTD_2D_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP>),
        nblocks234, nthreads_per_block234,
        shmem, res.get_stream(),
        hz, ex, ey, nx, ny );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_FDTD_2D_4));
...
RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(POLYBENCH_FDTD_2D, Hip, Base_HIP, Lambda_HIP, RAJA_HIP)
