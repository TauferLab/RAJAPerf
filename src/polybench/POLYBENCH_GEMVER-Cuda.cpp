...
void POLYBENCH_GEMVER::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_GEMVER_1));
      RPlaunchCudaKernel(
        (poly_gemver_1<GEMVER_THREADS_PER_BLOCK_TEMPLATE_PARAMS_CUDA>),
        nblocks1, nthreads_per_block1,
        shmem, res.get_stream(),
        A, u1, v1, u2, v2, n );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_GEMVER_1));

      size_t grid_size = RAJA_DIVIDE_CEILING_INT(n, block_size);

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_GEMVER_2));
      RPlaunchCudaKernel( (poly_gemver_2<block_size>),
                          grid_size, block_size,
                          shmem, res.get_stream(),
                          A, x, y, beta, n );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_GEMVER_2));

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_GEMVER_3));
      RPlaunchCudaKernel( (poly_gemver_3<block_size>),
                          grid_size, block_size,
                          shmem, res.get_stream(),
                          x, z, n );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_GEMVER_3));

      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(POLYBENCH_GEMVER_4));
      RPlaunchCudaKernel( (poly_gemver_4<block_size>),
                          grid_size, block_size,
                          shmem, res.get_stream(),
                          A, x, w, alpha, n );
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(POLYBENCH_GEMVER_4));
...
RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(POLYBENCH_GEMVER, Cuda, Base_CUDA, Lambda_CUDA, RAJA_CUDA)
