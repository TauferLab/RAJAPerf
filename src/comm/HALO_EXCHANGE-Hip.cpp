    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      for (Index_type l = 0; l < num_neighbors; ++l) {
        ...
        for (Index_type v = 0; v < num_vars; ++v) {
          ...
          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(HALO_EXCHANGE_pack_k));
          RPlaunchHipKernel( (halo_exchange_pack<block_size>),
                             nblocks, nthreads_per_block,
                             shmem, res.get_stream(),
                             buffer, list, var, len);
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(HALO_EXCHANGE_pack_k));
          buffer += len;
        }
      ...
      for (Index_type ll = 0; ll < num_neighbors; ++ll) {
        ...
        for (Index_type v = 0; v < num_vars; ++v) {
          ...
          dim3 nblocks((len + block_size-1) / block_size);
          constexpr size_t shmem = 0;
          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(HALO_EXCHANGE_unpack_k));
          RPlaunchHipKernel( (halo_exchange_unpack<block_size>),
                             nblocks, nthreads_per_block,
                             shmem, res.get_stream(),
                             buffer, list, var, len);
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(HALO_EXCHANGE_unpack_k));
          buffer += len;
        }
      }
      CAMP_HIP_API_INVOKE_AND_CHECK( hipStreamSynchronize, res.get_stream() );
