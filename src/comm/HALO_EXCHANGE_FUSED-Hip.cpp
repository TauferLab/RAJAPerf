...
void HALO_EXCHANGE_FUSED::runHipVariantDirect(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(HALO_EXCHANGE_FUSED_pack));
      RPlaunchHipKernel( (halo_exchange_fused_pack<block_size>),
                         ...
                         pack_var_ptrs,
                         pack_len_ptrs);
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(HALO_EXCHANGE_FUSED_pack));
      if (separate_buffers) {
        for (Index_type l = 0; l < num_neighbors; ++l) {
          Index_type len = pack_index_list_lengths[l];
      ...
      RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(HALO_EXCHANGE_FUSED_unpack));
      RPlaunchHipKernel( (halo_exchange_fused_unpack<block_size>),
                         ...
                         unpack_var_ptrs,
                         unpack_len_ptrs);
      RP_CALI_SUBKERNEL_END(RP_CALI_REGION(HALO_EXCHANGE_FUSED_unpack));
      CAMP_HIP_API_INVOKE_AND_CHECK( hipStreamSynchronize,  res.get_stream() );
