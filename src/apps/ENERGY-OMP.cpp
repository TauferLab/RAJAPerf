  ...
  switch ( vid ) {
    ...
    case Base_OpenMP : {
      ...
      for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
        ...
        #pragma omp parallel
        {
          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(ENERGY_1));
          #pragma omp for schedule(static) nowait
          for (Index_type i = ibegin; i < iend; ++i ) {
            ENERGY_BODY1;
          }
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(ENERGY_1));

          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(ENERGY_2));
          #pragma omp for schedule(static) nowait
          for (Index_type i = ibegin; i < iend; ++i ) {
            ENERGY_BODY2;
          }
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(ENERGY_2));

          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(ENERGY_3));
          #pragma omp for schedule(static) nowait
          for (Index_type i = ibegin; i < iend; ++i ) {
            ENERGY_BODY3;
          }
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(ENERGY_3));

          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(ENERGY_4));
          #pragma omp for schedule(static) nowait
          for (Index_type i = ibegin; i < iend; ++i ) {
            ENERGY_BODY4;
          }
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(ENERGY_4));

          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(ENERGY_5));
          #pragma omp for schedule(static) nowait
          for (Index_type i = ibegin; i < iend; ++i ) {
            ENERGY_BODY5;
          }
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(ENERGY_5));

          RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(ENERGY_6));
          #pragma omp for schedule(static) nowait
          for (Index_type i = ibegin; i < iend; ++i ) {
            ENERGY_BODY6;
          }
          RP_CALI_SUBKERNEL_END(RP_CALI_REGION(ENERGY_6));
