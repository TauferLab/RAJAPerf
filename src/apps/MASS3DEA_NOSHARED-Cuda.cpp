  ...
  case Base_CUDA: {
    ...
    if constexpr (tune_idx == 0) {
      ...
      for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
        ...
        RPlaunchCudaKernel((Mass3DEANoShared<block_size>), NE, nthreads_per_block, shmem,
                           res.get_stream(), B, D, M, Workspace);
    ...
    seq_for(gpu_block_sizes_type{}, [&](auto block_size) {
      ...
      if (run_params.numValidGPUBlockSize() == 0u ||
        ...
        if (vid == Base_CUDA) {
          ...
          addVariantTuning<&MASS3DEA_NOSHARED::runCudaVariantImpl<block_size, 0>>(
        ...
        if (vid == RAJA_CUDA) {
          ...
          addVariantTuning<&MASS3DEA_NOSHARED::runCudaVariantImpl<block_size, 1>>(
