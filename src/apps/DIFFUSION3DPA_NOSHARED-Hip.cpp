...
void DIFFUSION3DPA_NOSHARED::runHipVariantImpl(VariantID vid) {
  ...
  case Base_HIP: {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RPlaunchHipKernel( (Diffusion3DPANoShared<block_size>),
  ...
  case RAJA_HIP: {
    ...
    using launch_policy =
        RAJA::LaunchPolicy<RAJA::hip_launch_t<async, diff_ns::Q1D*diff_ns::Q1D*diff_ns::Q1D>>;

    using outer_x =
        RAJA::LoopPolicy<RAJA::hip_block_x_direct>;

    using inner_x =
        RAJA::LoopPolicy<RAJA::hip_thread_size_x_direct<diff_ns::Q1D>>;

    using inner_y =
        RAJA::LoopPolicy<RAJA::hip_thread_size_y_direct<diff_ns::Q1D>>;

    using inner_z =
        RAJA::LoopPolicy<RAJA::hip_thread_size_z_direct<diff_ns::Q1D>>;
  ...
  default: {
    ...
    getCout() << "\n DIFFUSION3DPA_NOSHARED : Unknown Hip variant id = " << vid
              << std::endl;
    break;
  }
  }
...
void DIFFUSION3DPA_NOSHARED::defineHipVariantTunings()
{
  for (VariantID vid : {Base_HIP, RAJA_HIP}) {
    seq_for(gpu_block_sizes_type{}, [&](auto block_size) {
      if (run_params.numValidGPUBlockSize() == 0u ||
          run_params.validGPUBlockSize(block_size)) {
        addVariantTuning<&DIFFUSION3DPA_NOSHARED::runHipVariantImpl<block_size, 1>>(
            vid, "block_"+std::to_string(block_size), Index_type(block_size));
        if (vid == RAJA_HIP) {
          addVariantTuning<&DIFFUSION3DPA_NOSHARED::runHipVariantImpl<block_size, 6>>(
