...
// Plane representation to reduce local memory load
template <HexHexPlane P>
RAJA_HOST_DEVICE RAJA_INLINE Real_type hexhex_plane_value(
    Real_type const x[hexhex_max_poly_vertices],
    Real_type const y[hexhex_max_poly_vertices],
    Real_type const z[hexhex_max_poly_vertices], Int_type const j) {
  switch (P) {
  case HexHexPlane::H2:
...
RAJA_HOST_DEVICE
RAJA_INLINE void hexhex_interp_zero(Real_type x[hexhex_max_poly_vertices],
                                    Real_type y[hexhex_max_poly_vertices],
                                    Real_type z[hexhex_max_poly_vertices],
                                    Int_type const out, Int_type const a,
                                    Int_type const b, Real_type const ca,
                                    Real_type const cb) {
...
clip_polygon_ge_0(Real_type x[hexhex_max_poly_vertices],
                  Real_type y[hexhex_max_poly_vertices],
                  Real_type z[hexhex_max_poly_vertices], Int_type &first,
                  Int_type &avail, unsigned long long &next_pack) {
  Int_type j = first;
  ...
  Int_type const first0 = first;
  Int_type j1 = -1, j2 = -1;
  Int_type jj1 = -1, jj2 = -1;

  Real_type c0 = hexhex_plane_value<P>(x, y, z, j);
  Real_type const c00 = c0;
  Real_type clast = c0;

// gives better performance than aggressive loop unrolling
#pragma unroll 1
  while (j >= 0) {
    Int_type jj = hexhex_next_get(next_pack, j);
    Int_type const jp = jj;
    if (jj < 0) {
      jj = first0;
    }

    Real_type const c1 = hexhex_plane_value<P>(x, y, z, jj);
  ...
  Int_type const jr1 = avail;
  avail = hexhex_next_get(next_pack, avail);
  hexhex_interp_zero(x, y, z, jr1, j1, jj1, hexhex_plane_value<P>(x, y, z, j1),
                     hexhex_plane_value<P>(x, y, z, jj1));

  Int_type const jr2 = avail;
  avail = hexhex_next_get(next_pack, avail);
  hexhex_interp_zero(x, y, z, jr2, j2, jj2, hexhex_plane_value<P>(x, y, z, j2),
                     hexhex_plane_value<P>(x, y, z, jj2));
...
#pragma unroll 1
  while (j >= 0) {
    Int_type const jp = hexhex_next_get(next_pack, j);
    if (hexhex_plane_value<P>(x, y, z, j) < Real_type(0.0)) {
...
RAJA_HOST_DEVICE
RAJA_INLINE void
cuda_hex_volpolyh_1poly(Real_type const x[hexhex_max_poly_vertices],
                        Real_type const y[hexhex_max_poly_vertices],
                        Real_type const z[hexhex_max_poly_vertices],
                        Int_type const first,
                        unsigned long long const next_pack, Real_type &vv,
  ...
  Real_type const x0 = x[j0];
  Real_type const y0 = y[j0];
  Real_type const z0 = z[j0];

  Int_type const j1 = hexhex_next_get(next_pack, j0);
  if (j1 < 0) {
    return;
  }
...
#pragma unroll 1
  while (j2 >= 0) {
    Real_type const x2 = x[j2];
    Real_type const y2 = y[j2];
    Real_type const z2 = z[j2];
    Real_type const dx2 = x2 - x0;
    Real_type const dy2 = y2 - y0;

    Real_type const area2 = (dx1 * dy2 - dx2 * dy1);
    Real_type const v0 = (z0 + z1 + z2) * area2;
    vv += v0;
    ...
    Real_type &vz_thr) // z moment contribution for this triangle-tet
{
  ...
  Real_type const det = xtt[1] * ytt[2] * ztt[3] - xtt[1] * ytt[3] * ztt[2] +
                        xtt[2] * ytt[3] * ztt[1] - xtt[2] * ytt[1] * ztt[3] +
                        xtt[3] * ytt[1] * ztt[2] - xtt[3] * ytt[2] * ztt[1];
  Real_type const deti = det / (det * det + 1.0e-100);

  // Cross products.
  Real_type const cyz0 = ytt[2] * ztt[3] - ztt[2] * ytt[3];
  Real_type const czx0 = ztt[2] * xtt[3] - xtt[2] * ztt[3];
  Real_type const cxy0 = xtt[2] * ytt[3] - ytt[2] * xtt[3];

  //   Coordinates of the facet in the transformed frame.
  xa0 = (xdt[0] - xtt[0]) * cyz0 + (ydt[0] - ytt[0]) * czx0 +
        (zdt[0] - ztt[0]) * cxy0;
  xa1 = (xdt[1] - xtt[0]) * cyz0 + (ydt[1] - ytt[0]) * czx0 +
        (zdt[1] - ztt[0]) * cxy0;
  xa2 = (xdt[2] - xtt[0]) * cyz0 + (ydt[2] - ytt[0]) * czx0 +
        (zdt[2] - ztt[0]) * cxy0;

  Real_type const cyz1 = ytt[3] * ztt[1] - ztt[3] * ytt[1];
  Real_type const czx1 = ztt[3] * xtt[1] - xtt[3] * ztt[1];
  Real_type const cxy1 = xtt[3] * ytt[1] - ytt[3] * xtt[1];

  ya0 = (xdt[0] - xtt[0]) * cyz1 + (ydt[0] - ytt[0]) * czx1 +
        (zdt[0] - ztt[0]) * cxy1;
  ya1 = (xdt[1] - xtt[0]) * cyz1 + (ydt[1] - ytt[0]) * czx1 +
        (zdt[1] - ztt[0]) * cxy1;
  ya2 = (xdt[2] - xtt[0]) * cyz1 + (ydt[2] - ytt[0]) * czx1 +
        (zdt[2] - ztt[0]) * cxy1;

  Real_type const cyz2 = ytt[1] * ztt[2] - ztt[1] * ytt[2];
  Real_type const czx2 = ztt[1] * xtt[2] - xtt[1] * ztt[2];
  Real_type const cxy2 = xtt[1] * ytt[2] - ytt[1] * xtt[2];

  za0 = (xdt[0] - xtt[0]) * cyz2 + (ydt[0] - ytt[0]) * czx2 +
        (zdt[0] - ztt[0]) * cxy2;
  za1 = (xdt[1] - xtt[0]) * cyz2 + (ydt[1] - ytt[0]) * czx2 +
        (zdt[1] - ztt[0]) * cxy2;
  za2 = (xdt[2] - xtt[0]) * cyz2 + (ydt[2] - ytt[0]) * czx2 +
        (zdt[2] - ztt[0]) * cxy2;
  ...
  Real_type xa[hexhex_max_poly_vertices];
  Real_type ya[hexhex_max_poly_vertices];
  Real_type za[hexhex_max_poly_vertices];
  ...
  clip_polygon_ge_0<HexHexPlane::H2>(xa, ya, za, first, avail, next_pack);
  if (first < 0) {
    return;
  }

  clip_polygon_ge_0<HexHexPlane::X>(xa, ya, za, first, avail, next_pack);
  if (first < 0) {
    return;
  }

  clip_polygon_ge_0<HexHexPlane::Y>(xa, ya, za, first, avail, next_pack);
  if (first < 0) {
    return;
  }

  clip_polygon_ge_0<HexHexPlane::Z>(xa, ya, za, first, avail, next_pack);
  if (first < 0) {
    return;
  }
  ...
  clip_polygon_ge_0<HexHexPlane::H>(xa, ya, za, first, avail, next_pack);
  ...
  clip_polygon_ge_0<HexHexPlane::NEG_H>(xa, ya, za, first, avail, next_pack);

#pragma unroll 1
  for (Int_type j = first; j >= 0; j = hexhex_next_get(next_pack, j)) {
...
RAJA_HOST_DEVICE
RAJA_INLINE void
hex_intsc_subz(Real_const_ptr const xds, //  [24] donor subzone coords
               Real_const_ptr const xts, //  [24] target subzone coords
               ...
               Real_type &vz_thr) // z moment contribution for this triangle-tet
{
  Real_const_ptr const yds = xds + 8;
  Real_const_ptr const zds = yds + 8;

  Real_const_ptr const yts = xts + 8;
  Real_const_ptr const zts = yts + 8;
  ...
  Int_type const cyc_nod[len_cycnod] = {1, 5, 4, 6, 2, 3, 1};

  // which subzone vertices form the triangular facet.
  bool const low_facet = dfacet < 6;
  Int_type const dv0 = low_facet ? 0 : 7;
  Int_type const dv1 =
      low_facet ? cyc_nod[dfacet] : cyc_nod[n_dfacets - dfacet];
  Int_type const dv2 =
      low_facet ? cyc_nod[dfacet + 1] : cyc_nod[n_dfacets - dfacet - 1];

  //  Donor triangle coordinates.
  Real_type const xdt[3] = {xds[dv0], xds[dv1], xds[dv2]};
  Real_type const ydt[3] = {yds[dv0], yds[dv1], yds[dv2]};
  Real_type const zdt[3] = {zds[dv0], zds[dv1], zds[dv2]};
  ...
  Int_type const vert_cyc[6] = {1, 3, 2, 6, 4, 5};

  Int_type const v1 = vert_cyc[ttet];
  xtt[1] = xts[v1];
  ytt[1] = yts[v1];
  ztt[1] = zts[v1];
  Int_type const v2 = vert_cyc[(ttet + 1) % 6];
  xtt[2] = xts[v2];
  ytt[2] = yts[v2];
  ztt[2] = zts[v2];
  xtt[3] = xts[7];
  ...
  cuda_intsc_tri_tet(xdt, ydt, zdt, xtt, ytt, ztt, vv_thr, vx_thr, vy_thr,
                     vz_thr);
}
...
#define INTSC_HEXHEX_BODY_SEQ                                                  \
  ...
  if (ipair < nisc_stage) {                                                    \
    ...
    hex_intsc_subz(xds, xts, dfacet, ttet, vv_lo, vx_lo, vy_lo, vz_lo);        \
  }                                                                            \
