  ...
  switch ( vid ) {
    ...
    case Base_Seq : {
      ...
      for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
        ...
        RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(INTSC_HEXHEX_1));
        for (Index_type i = ibegin ; i < iend ; ++i ) {
          INTSC_HEXHEX_SEQ ( i, iend ) ;
        }
        RP_CALI_SUBKERNEL_END(RP_CALI_REGION(INTSC_HEXHEX_1));
        RP_CALI_SUBKERNEL_BEGIN(RP_CALI_REGION(INTSC_HEXHEX_2));
        for (Index_type i = ibegin ; i < n_szpairs ; ++i ) {
          FIXUP_VV_BODY ;
        }
        RP_CALI_SUBKERNEL_END(RP_CALI_REGION(INTSC_HEXHEX_2));
