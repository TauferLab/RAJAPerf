                      ...
                      Index_type ibegin, Index_type iend)
{
   ...
   if (i < iend) {
     ...
     NODAL_ACCUMULATION_3D_BODY(RAJAPERF_ATOMIC_ADD_CUDA);
   }
}
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RAJA::forall< RAJA::cuda_exec<block_size, true /*async*/> >( res,
        zones, [=] __device__ (Index_type i) {
          NODAL_ACCUMULATION_3D_BODY(RAJAPERF_ATOMIC_ADD_RAJA_CUDA);
      });
      RP_CALI_SUBKERNEL_END("NODAL_ACCUMULATION_3D_1");
...
RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(NODAL_ACCUMULATION_3D, Cuda, Base_CUDA, RAJA_CUDA)
