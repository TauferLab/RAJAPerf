...
__global__ void Mass3DEANoShared(const Real_ptr B, const Real_ptr D, Real_ptr M,
                                 Real_ptr Workspace) {
...
void MASS3DEA_NOSHARED::runHipVariantImpl(VariantID vid)
{
  ...
  case Base_HIP: {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RPlaunchHipKernel( (Mass3DEANoShared<block_size>),
    ...
    seq_for(gpu_block_sizes_type{}, [&](auto block_size) {
      ...
      if (run_params.numValidGPUBlockSize() == 0u ||
        ...
        if (vid == Base_HIP) {
          ...
          addVariantTuning<&MASS3DEA_NOSHARED::runHipVariantImpl<block_size, 0, 1>>(
        ...
        if (vid == RAJA_HIP) {
          ...
          addVariantTuning<&MASS3DEA_NOSHARED::runHipVariantImpl<block_size, 1, 1>>(
              vid, "cached_block_stride_loop_" + std::to_string(block_size), Index_type(block_size));

          addVariantTuning<&MASS3DEA_NOSHARED::runHipVariantImpl<block_size, 1, 6>>(
